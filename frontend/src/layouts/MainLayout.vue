    ...
    <q-header class="glass-header text-white" height-hint="64">
      <q-toolbar class="q-px-lg" style="height: 64px;">
        ...
        <div class="row items-center q-gutter-md">
          ...
          <div v-if="auth.token" class="row items-center q-gutter-sm cursor-pointer profile-btn q-pa-xs rounded-borders">
            <q-avatar size="32px" color="primary" text-color="white" class="shadow-glow">
              {{ auth.user?.username?.charAt(0).toUpperCase() || 'U' }}
            </q-avatar>
            <div class="gt-xs text-left q-ml-xs">
              <div class="text-caption text-weight-bold leading-none">@{{ auth.user?.username || 'User' }}</div>
              <div class="text-grey-5" style="font-size: 10px; line-height: 1;">{{ auth.user?.role || 'Guest' }}</div>
            </div>
            <q-icon name="arrow_drop_down" size="20px" class="text-grey-4 gt-xs" />

            <q-menu auto-close class="glass-menu text-white" transition-show="jump-down" transition-hide="jump-up">
    ...
    >
      <div class="column justify-between h-full py-md">
        ...
        <div>
          ...
          <div class="q-px-lg q-pt-md q-pb-sm text-grey-5 text-weight-bold text-uppercase tracking-wider" style="font-size: 10px;">
            Main Navigation
          </div>
          ...
          <q-list class="q-px-sm q-gutter-y-xs">
            ...
            >
              ...
              <q-item-section>
                ...
                <q-item-label caption class="text-grey-5 sidebar-caption">{{ link.caption }}</q-item-label>
              </q-item-section>
            </q-item>
          </q-list>
        </div>
...
const linksList = computed(() => [
  ...
  },
  {
    ...
    caption: 'Real-time CRUD Comments',
    icon: 'forum',
    link: '/comments',
  },
  {
    title: t('nav.tags'),
    caption: 'Real-time CRUD Tags',
    icon: 'local_offer',
    link: '/tags',
  },
])
...
.sidebar-item-active {
  background: linear-gradient(135deg, rgba(0, 180, 219, 0.15) 0%, rgba(0, 131, 176, 0.05) 100%) !important;
  color: #00b4db !important;
